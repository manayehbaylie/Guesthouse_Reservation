...
export const GuesthouseDetails = ({
  ...
  const [activeImage, setActiveImage] = useState(guesthouse.images[0] || '');

  useEffect(() => {
    const fetchRooms = async () => {
      setLoading(true);
      try {
        const roomList = await ApiService.getRoomsForGuesthouse(guesthouse.id);
        setRooms(roomList);
      } catch (err) {
        console.error(err);
      } finally {
  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <div className="bg-white rounded-3xl border border-stone-200 overflow-hidden shadow-sm p-6 sm:p-8 space-y-6">
            ...
            {guesthouse.images.map((img, idx) => (
              <button
                ...
                className={`relative h-20 sm:h-28 rounded-xl overflow-hidden border-2 transition-all shrink-0 md:shrink ${
                  activeImage === img ? 'border-amber-700 ring-2 ring-amber-500/20' : 'border-transparent opacity-70 hover:opacity-100'
                }`}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-4">
          <div className="lg:col-span-2 space-y-4">
            <h2 className="text-lg font-bold font-serif text-stone-900">About {guesthouse.name}</h2>
            <p className="text-sm text-stone-700 leading-relaxed">
              {guesthouse.description}
            </p>

            <div className="pt-4 border-t border-stone-100">
              <h3 className="text-xs font-bold uppercase tracking-wider text-stone-400 mb-3">Property Amenities</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {guesthouse.amenities.map((am) => (
                  <div key={am} className="flex items-center gap-2 text-xs font-medium text-stone-700 bg-stone-50 p-2.5 rounded-xl border border-stone-200/80">
          ...
          <div className="bg-amber-50/60 p-6 rounded-2xl border border-amber-200/80 space-y-4 h-fit">
            <div className="flex items-center gap-2 text-amber-900 font-bold text-sm">
              <ShieldCheck className="w-5 h-5 text-amber-800" /> Automated Booking Protection
            </div>
            <ul className="text-xs text-amber-950 space-y-2.5 leading-relaxed">
              <li className="flex items-start gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-amber-800 mt-1.5 shrink-0"></span>
                <span><strong>No Double Booking:</strong> Rooms are automatically locked in real-time upon verified payment.</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-amber-800 mt-1.5 shrink-0"></span>
                <span><strong>Accepted Payments:</strong> Telebirr, Chapa, CBE, Visa/Mastercard.</span>
              </li>
              <li className="flex items-start gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-amber-800 mt-1.5 shrink-0"></span>
                <span><strong>Instant Confirmation:</strong> System generates downloadable printable receipt & voucher immediately.</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold font-serif text-stone-900">Available Accommodations & Rooms</h2>
            <p className="text-xs text-stone-500">Select a room to proceed with online booking and instant confirmation.</p>
          </div>
        ...
        {loading ? (
          <div className="text-center py-12 text-stone-500">Loading rooms...</div>
        ) : rooms.length === 0 ? (
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-start gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${
                            isAvailable ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-red-100 text-red-800'
                          }`}>
                            {isAvailable ? 'Available Now' : 'Unavailable'}
                          </span>
                        </div>
                        <h3 className="font-bold text-stone-900 text-lg mt-1">{room.type}</h3>
                      </div>
                      ...
                      <div className="text-right">
                        ...
                        <span className="block text-[10px] text-stone-500 font-normal">ETB / night</span>
                      </div>
                    </div>

                    {/* Room Photo if present */}
                    {room.image && (
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-xs text-stone-500 border-t border-stone-100 pt-3">
                      ...
                      {room.amenities && room.amenities.map((ram) => (
                        <span key={ram} className="text-[10px] bg-stone-100 text-stone-600 px-2 py-0.5 rounded">
                          {ram}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="pt-5 mt-4 border-t border-stone-100 flex justify-end">
                    <button
                      ...
                      onClick={() => onBookRoom(guesthouse, room)}
                    ...
                    >
                      ...
                      {isAvailable ? 'Book & Pay Online' : 'Currently Unavailable'}
                    </button>
                  </div>
